...
function createChart(resultJSON, number) {
    // create data
    var data = [];
    ...
    for (var i = 0; i < parsed.length; i++) {
        if (number == 1) {
            data.push({
                ...
                velo: parsed[i].kecepatan_1,
                dire: parsed[i].arah_1
            });
        } else if (number == 2) {
            data.push({
                time: parsed[i].timestamp,
                velo: parsed[i].kecepatan_2,
                dire: parsed[i].arah_2
            });
        } else if (number == 3) {
            data.push({
                time: parsed[i].timestamp,
                velo: parsed[i].kecepatan_3,
                dire: parsed[i].arah_3
            });
        }
    }
    ...
    series2.name('Arah');
    series2.zIndex(-1);
    series1.tooltip().valuePostfix(" m/s");
    ...
    // chart.getSeries(0).name("Kecepatan dalam m/s");
    // chart.getSeries(1).name("Arah angin dalam derajat");
